...
#define TOUCH_CHANNEL_3        (10)
#define TOUCH_CHANNEL_4        (14)

#define LIGHT_TOUCH_THRESHOLD  (0.01)
...
static void touch_event_light_4(void *arg, void *data)
{
    button_event_t event = iot_button_get_event(arg);
    ESP_LOGI(TAG, "Light Button 4: %s", iot_button_get_event_str(event));
}
...
static void touch_task(void *arg)
{
    ...
    // Create light press button
    button_handle_t btn_light_4 = NULL;
    ret = iot_button_new_touch_button_device(&btn_cfg, &touch_cfg_4, &btn_light_4);
    assert(ret == ESP_OK);
    ...
    iot_button_register_cb(btn_light_2, BUTTON_PRESS_DOWN, NULL, touch_event_light_2, NULL);
    // iot_button_register_cb(btn_light_2, BUTTON_SINGLE_CLICK, NULL, touch_event_light_2, NULL);
    // iot_button_register_cb(btn_light_2, BUTTON_PRESS_REPEAT, NULL, touch_event_light_2, NULL);

    // Register touch IO13 callback
    iot_button_register_cb(btn_light_3, BUTTON_PRESS_DOWN, NULL, touch_event_light_3, NULL);
    // iot_button_register_cb(btn_light_3, BUTTON_PRESS_DOWN, NULL, touch_event_light_3, NULL);

    // Register touch IO14 callback
    iot_button_register_cb(btn_light_4, BUTTON_PRESS_DOWN, NULL, touch_event_light_4, NULL);
    ...
    while (1) {
        ...
        //printf("Touch task free stack: %u bytes\n", free_bytes);
    }
}

void app_main(void)
{
    //测试esp32s3支持触摸的GPIO 8/9/10/14
